...
export async function updateShiftTotalStudents(shiftId: string, totalStudentsRegistered: number) {
  ...
  await API.patch(`/shifts/${shiftId}`, { totalStudentsRegistered: validTotal });
}
...
export async function getAvailableRooms(shiftId: string, day: string, startHour: string, endHour: string) {
  const shiftsResponse = await API.get('/shifts');
  const classroomsResponse = await API.get('/classrooms');
  const buildingsResponse = await API.get('/buildings');
  ...
  const occupiedRooms = allShifts
    .filter(
      (s: any) =>
        s.id !== shiftId &&
        s.day === day &&
        ((s.from >= startHour && s.from < endHour) || (s.to > startHour && s.to <= endHour))
    )
  ...
  return allClassrooms
    ...
    .map((room: any) => {
      const building = allBuildings.find((b: any) => String(b.id) === String(room.buildingId));
      return {
        id: room.id,
...
export async function getRoomByShiftId(shiftId: string) {
  ...
  const room = roomResponse.data.find((r: any) => String(r.id) === String(shift.classroomId));
  ...
  const building = buildingResponse.data.find((b: any) => String(b.id) === String(room.buildingId));
...
export async function createAllocation(allocation: { id: string; shiftId: Number; studentId: string }) {
